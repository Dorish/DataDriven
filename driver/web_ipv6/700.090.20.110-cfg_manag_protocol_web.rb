...
begin 
  ...
  while(row <= rows)
    ...
    g.wrt_checkbox(g.snmp_v3, ws.Range("l#{row}")['Value'])
    ...
    g.edit.click
    puts "  snmpv1v2 agent  = " + ws.Range("bc#{row}")['Value'] = g.checkbox(g.snmp_v1v2)
    puts "  snmpv3 agent = " + ws.Range("bd#{row}")['Value'] = g.checkbox(g.snmp_v3)
    g.save.click_no_wait
    g.jsClick('OK')
