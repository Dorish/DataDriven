...
begin
  ...
  while(row <= rows)
    ...
    puts "#{dnsmode}"
    g.dns_ipv6mode(dnsmode).set

    # Write DNS Pri Server and Sec Address Seletion
    if (g.dns_ipv6mode(2).checked? == true)
      g.dns_ipv6addr1.set(ws.Range("l#{row}")['Value'].to_s)
      g.dns_ipv6addr2.set(ws.Range("m#{row}")['Value'].to_s)
    end
    ...
    if (g.dns_ipv6mode(2).checked? == true)
      ws.Range("bc#{row}")['Value'] = "2"
	 	  ws.Range("bd#{row}")['Value'] = g.dns_ipv6addr1.value
      ws.Range("be#{row}")['Value'] = g.dns_ipv6addr2.value
    elsif (g.dns_ipv6mode(1).checked? == true)
