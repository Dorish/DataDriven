...
def snmptrip_link; uav.link(:text, 'SNMPv1/v2c Trap Settings (20)');end
def snmpv1v2trip(val); uav.link(:text, "SNMPv1/v2c Trap Settings [#{val}]");end
...
def targetadd; tav.text_field(:id, 'str42');end
def trapport; tav.text_field(:id, 'num43');end
def community; tav.text_field(:id, 'str51');end
...
begin
  ...
  for row in 1..rows
    ...
    #Open trip number page
    snmpv1v2trip(number).click
    ...
    targetadd.set(ws.Range("k#{row}")['Value'].to_s)

    #2 SNMP Trap Port
    puts "2 SNMP Trap Port"
    trapport.set(ws.Range("L#{row}")['Value'].to_s)
  ...
  for i in 1..rows
    ...
    ws.Range("t#{i+1}")['Value'] = targetadd.value.to_s

    #2 read SNMP Trap Port
    ws.Range("u#{i+1}")['Value'] = trapport.value.to_s
