...
This version writes all of the snmp data point to an array and then the array
...
while data_row <= iter +1
  ...
  alldata = [] # initialize array
  while (row <= rows +1)
    oid  = ws.range("k#{row}")['Value']
    data = snmpget(com,ip,mib,oid)
    alldata = alldata.to_a + data.split(/ /).push
    data_col = data_col.next
    row += 1
  end
  ws.Range("af#{data_row}:#{data_col}#{data_row}")['Value'] = alldata
