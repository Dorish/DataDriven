...
Dir.open(Dir.pwd).each do |fname|      # read log file directory
  ...
  else if  fname =~ /^10.*adapt.txt/               # parse each txt file that starts with "10"
    card_type = "Web_Adapt"
    parse_key = 'parse_adapt_key.txt'            # file that contains items to parse from log
    else
      next
    end
  end
  ...
  File.open(outfile, 'w') do |f|      # open csv file
      ...
      open(infile).each do|line|        # read input file lines
        ...
        regexp = /^#{key}[\s*:]+|\d{4}[-\d{2}]+T[\d{2}:]+.\d{3}\+[\d{2}:]+\s*#{key}\s*-*[\w]+\(\d{5}+\)[\s*:]+/
        if line =~  regexp         # get the lines in the log that match the one of the parse keys
           ...
           if card_type == "Unity" # for unity card, it have the different format with adapt logging, one txt line write to a line in csv line.
