...
$desired_location = "4.3"

  # close the connection for WEB Adapt
def tn_adapt_close(telnet)
...
begin
  ...
  csv_file = File.dirname(__FILE__) + '\\' + 'telnet_logging_adapt.csv' # use this path when we run the script in ruby environment
  input_parameter_list = Array.new()
